...
public class ShippingLabelPdfGenerator {
    ...
    public File generateShippingLabel(List<Long> tripIds, List<ShippingLabelsDto> dtos, File file)
            throws DispatchDocumentsException {
        int startCount = 0;
        int endCount = 0;
        File page = new File("label_" + startCount + ".pdf");
        List<File> pages = new ArrayList<>();
        List<ShippingLabelsDto> dtoPages = new ArrayList<>();
        try {
            FileOutputStream stream = new FileOutputStream(page);
            for (ShippingLabelsDto dto : dtos) {
            ...
            PDFUtils.mergePdf(pages, outputStream);
            outputStream.flush();
            outputStream.close();
            log.info("Shipping label is generated");
        } catch (IOException | DocumentException ex) {
            log.error("Something went wrong in shipping label generator....\n" + ex.getMessage());
            if (file.exists()) file.delete();
            throw new DispatchDocumentsException(ex.getMessage());
        } finally {
    ...
    private void createPage(List<ShippingLabelsDto> dtos, FileOutputStream stream, int startCount)
        ...
        for (ShippingLabelsDto dto : dtos) {
            ...
            table.addCell(createAwb(cb, dto));
    ...
    private static PdfPCell createTrip(ShippingLabelsDto dto) {
    ...
    private static PdfPCell createAwb(PdfContentByte cb, ShippingLabelsDto dto) {
        ...
        try {
            ...
            throw new DispatchDocumentsException(e.getMessage());
        } finally {
            if (outputFile.exists()) outputFile.delete();
        }
    }

    private static PdfPCell createSequence(PdfContentByte cb, ShippingLabelsDto dto) {
        ...
        cell.setCellEvent(
                (cell1, position, canvases) -> {
                    ...
                    double xCor = xLeft + ((cellWidth * 2) / 3.0f) + 10.0f;
                    double yCor = yLeft + (cellHeight / 2.0f) - 3.0f;
    ...
    private static PdfPCell createPackage(ShippingLabelsDto dto) {
    ...
    private static PdfPCell createShippingAddress(ShippingLabelsDto dto) {
        ...
        innerCell = new PdfPCell(new Phrase(dto.getUserDetails().getName(), font));
        innerCell.setBorder(Rectangle.NO_BORDER);
        innerCell.setVerticalAlignment(Element.ALIGN_TOP);
        innerCell.setHorizontalAlignment(Element.ALIGN_LEFT);
        cellTable.addCell(innerCell);
        ...
        innerCell =
                new PdfPCell(
                        new Phrase(dto.getUserDetails().getLocation().getAddress().getAddress(), font));
        ...
        innerCell = new PdfPCell(new Phrase("Contact: " + dto.getUserDetails().getPhoneNumber(), font));
        innerCell.setBorder(Rectangle.NO_BORDER);
        innerCell.setVerticalAlignment(Element.ALIGN_TOP);
        innerCell.setHorizontalAlignment(Element.ALIGN_LEFT);
        ...
        innerCell =
                new PdfPCell(
                        new Phrase(
                                "PIN:" + dto.getUserDetails().getLocation().getAddress().getPincode(), font));
        innerCell.setBorder(Rectangle.NO_BORDER);
        innerCell.setVerticalAlignment(Element.ALIGN_TOP);
    ...
    private static PdfPCell createVendorAddress(ShippingLabelsDto dto) {
    ...
    public static PdfPCell createFooter(ShippingLabelsDto dto) {
